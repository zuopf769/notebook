...
/**
 * 继承机制提供的一个基类，用户可以通过继承lang.Class来获取它的属性及方法。
 * @class
 * @name 	lang.Class
 ...
 * 为支持老版本的className属性，以后统一改成 __type
 ...
 * @return  {object}            实例对象
 */
 window["compInstance"] = function(guid) {
     return $$._instances[guid];
 }

//  添加 compInstance 这个全局方法，可以快速地通过guid得到实例对象
//  废除创建类时指定guid的模式，guid只作为只读属性
//  废除lang._instances 模块，由统一的global机制完成；
...
Class.prototype.dispatchEvent = function (event, options) {
    ...
    // 添加本方法的第二个参数，将 options extend到event中去传递
    options = options || {};
 ...
 * @name createClass
 * @function
 * @grammar .createClass(constructor[, options])
 * @param {Function} constructor 类的构造器函数
 * @param {Object} [options]

 * @config {string} [type] 类名
 * @config {Function} [superClass] 父类，lang.Class
 * @version 1.2
 * @remark
